...
@on_natural_language(keywords={'咋打钱', '咋给钱', '怎样给钱', '给别人钱'}, only_to_me=False)
async def _(session: NLPSession):
...
@on_natural_language(keywords={'添加朋友', '添加人', '添加好友'}, only_to_me=False)
async def _(session: NLPSession):
...
async def _(session: CommandSession):
    await session.send('域名/ip中的域名/ip')
...
@on_command('光影', aliases={'装光影'})
async def _(session: CommandSession):
    await session.send('群文件-mc资源里有光影包')
    await session.send('把光影包(那个压缩包)放进启动器目录\.minecraft\shaderpacks文件夹里面')
    await session.send('然后在esc菜单-视频设置-光影里面选择光影')

@on_natural_language(keywords={'用光影', '光影', '上光影', '加光影'}, only_to_me=False)
async def _(session: NLPSession):
    return IntentCommand(90.0, '光影')

@on_command('材质包', aliases={'装材质包'})
async def _(session: CommandSession):
    await session.send('群文件-mc资源里有材质包')
    await session.send('把材质包包(那个压缩包)放进启动器目录\.minecraft\resourcepacks文件夹里面')
    await session.send('然后在esc菜单-视频设置-资源包/材质包里面选择材质包')

@on_natural_language(keywords={'用材质包', '材质包', '上材质包', '加材质包'}, only_to_me=False)
async def _(session: NLPSession):
    return IntentCommand(90.0, '材质包')
...
@on_natural_language(keywords={'fz不在家', '老公不在家'}, only_to_me=False, only_short_message=False, permission=lambda sender: sender.is_superuser)
async def _(session: NLPSession):
...
Fantasy_permission = lambda sender: sender.sent_by(self, 2123775697 | Container[2118630546])
@on_natural_language(keywords={'老婆不在家', 'ax不在家'}, only_to_me=False, only_short_message=False, permission=Fantasy_permission)
async def _(session: NLPSession):
